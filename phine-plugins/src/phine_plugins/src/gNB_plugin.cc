 ...
 * - Parsing required parameters from the SDF, such as link name, network name,
 * IP addresses, MCC, MNC, and debug flag.
 * - Fetching the PROJECT_PATH environment variable to locate configuration
 * files.
 * - Modifying Docker Compose and configuration files with the parsed parameters
 * to set up the gNB instance.
 * - Launching the Docker Compose setup for the gNB.
 * - Restoring the original service name in the Docker Compose file for
 * subsequent gNBs.
 *
 ...
 * parameters.
 * @param[in,out] _ecm The EntityComponentManager for accessing and modifying
 * simulation entities.
 * @param[in,out] _eventMgr The EventManager (unused).
 *
 * @note The method logs errors and returns early if required parameters are
 * missing or if initialization fails.
 * @note The method modifies files on disk and launches system commands; use
...
void gNB_plugin::Configure(const gz::sim::Entity &_entity,
	...
	RoboSimLogger::Log(LogLevel::ERR,
			   "gNB_plugin plugin should be attached to a model "
			   "entity. Failed to initialize.\n");
	return;
    }

    // Get the model's name (this should be "gNB{i}" with i the number of the
    // gNB as in the SDF definition)
	...
	"pose_of_" + this->model_name, 10);

    // Clone the SDF to parse the plugin parameters
    auto sdfClone = _sdf->Clone();

    // Get the CN type from SDF (defaults to "oai" for backward compatibility)
    GetOptionalParamFromSDF(sdfClone, "cn_type", this->cn_type, "oai");
    ...
    // Parse free5gc-specific parameters when needed
    if (this->cn_type == "free5gc") {
	if (!GetParamFromSDF(sdfClone, "IP_GNB_N3", this->ip_gnb_n3))
	    return;
	if (!GetParamFromSDF(sdfClone, "n3_net_name", this->n3_net_name))
	    return;
    }
...
void gNB_plugin::configureOAI(const char *project_path) {
	...
	"container_name"; // The parameter key to search for
    const std::string new_value3 =
	"oai-" +
    ...
    // Reset the old service name in the Docker Compose file to be access by
    // other gNBs
...
void gNB_plugin::configureFree5gc(const char *project_path) {
    ...
    modify_dockerC(file_path2, "GNB_IPV4_ADDRESS_FOR_NG_AMF", this->ip_gnb,
		   this->debug_logs);
    modify_dockerC(file_path2, "GNB_IPV4_ADDRESS_FOR_NGU", this->ip_gnb_n3,
		   this->debug_logs);
...
/**
 * @brief Called after each simulation update to publish the pose of the gNB
 * link.
 *
 * This method ensures that ROS is initialized, locates the specified link
 * entity if it has not already been found, retrieves its world pose, formats
 * the pose information into a string, and publishes it as a ROS message.
 *
 * @param[in] _info Simulation update information.
 * @param[in] _ecm Entity-component manager providing access to simulation
 * entities.
 */
void gNB_plugin::PostUpdate(const gz::sim::UpdateInfo &_info,
    ...
    oss << "Position: [" << rawPose.Pos().X() << ", " << rawPose.Pos().Y()
	<< ", " << rawPose.Pos().Z() << "], "
	<< "Orientation: [" << rawPose.Rot().Roll() << ", "
	<< rawPose.Rot().Pitch() << ", " << rawPose.Rot().Yaw() << "]";
