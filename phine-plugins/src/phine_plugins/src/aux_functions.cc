 ...
 * @return A string representing the log level ("DEBUG", "WARN", "ERR", "INFO", or "UNKNOWN" for unrecognized values).
 */
 ...
 *
 * @param level The log level for which to retrieve the color code. Supported values are:
 *              - LogLevel::DEBUG: Blue
 *              - LogLevel::WARN: Yellow
 *              - LogLevel::ERR: Red
 *              - LogLevel::INFO: Green
 * @return A string containing the ANSI escape code for the color associated with the given log level.
 *         Returns the default color code ("\033[0m") if the log level is unrecognized.
 */
...
/**
 * @brief Logs a message with a specified log level, applying color formatting to the output.
 *
 * Outputs the log message to the standard output stream, prefixing it with the log level and
 * applying a color code based on the log level for better readability in the console.
 *
...
void RoboSimLogger::Log(LogLevel level, const std::string& message) {
    ...
    // Output the entire log message with the color code applied to the whole line
    std::cout << color_code << "[RoboSim5g] [" << level_str << "] " << message << "\033[0m" << std::endl;
}

/**
 * @brief Modifies the value associated with a specified key in a file.
 *
 * This function searches for a line in the file at `file_path` that contains the given `key`
 * followed by a colon and a value, and replaces the value with `new_value`. The function preserves
 * the original indentation of the matched line. If `debug` is true, debug messages are logged.
 *
...
void modify_dockerC(const std::string& file_path, const std::string& key, const std::string& new_value, bool debug) {
    ...
    if (!file_in) {
        RoboSimLogger::Log(LogLevel::ERR, "Error opening file for reading: " + file_path);
        return;
    }
    ...
    if (!file_out) {
        RoboSimLogger::Log(LogLevel::ERR, "Error opening file for writing: " + file_path);
        return;
    }
 ...
 *
 * This function searches for a line in the given file that matches the specified key
 * (in the form "key = value") and replaces its value with the provided new value.
 * The function preserves all other lines and writes the modified content back to the file.
 *
 ...
 *
 * @note If the file cannot be opened for reading or writing, an error is logged and the function returns.
...
void modify_conf(const std::string& file_path, const std::string& key, const std::string& new_value, bool debug) {
    ...
    // Create a regex pattern to match the key followed by an equals sign and its value
    std::string regex_pattern = R"(\b)" + key + R"(\b\s*=\s*\S+)";    
    ...
    while (std::getline(file_in, line)) {
        ...
        if (std::regex_search(line, pattern)) {
            ...
            if (std::regex_search(line, match, std::regex(R"((.*)\s*=\s*)"))) {
                ...
                // Replace the key's value with the new value, enclosing it in quotes
...
/**
 * @brief Modifies a specific key in a file, replacing it with a new key while preserving its value.
 *
 * This function searches for lines in the specified file that contain the given `old_key` followed by a colon,
 * and replaces the key with `new_key`, keeping any associated value intact. The function preserves indentation
 * and only updates lines where the key matches exactly. The modified content is written back to the original file.
 *
 * @param file_path The path to the file to be modified.
 ...
 *
 * @note If the file cannot be opened for reading or writing, an error is logged and the function returns early.
 */
void modify_service_name(const std::string& file_path, const std::string& old_key, const std::string& new_key, bool debug) {
    ...
    // Create a regex pattern to find the old_key followed by a colon and optional spaces or value
    std::string regex_pattern = R"(\b)" + old_key + R"(\b\s*:\s*)"; // Matches `old_key:` with optional spaces after it
    ...
    while (std::getline(file_in, line)) {
        ...
        if (std::regex_search(line, pattern)) {
            ...
            std::smatch match;
            if (std::regex_search(line, match, std::regex(R"(^\s*)"))) {
                std::string indentation = match.str(0);  // Capture leading whitespace
                
                // If there is a value after the colon, retain the value. Otherwise, keep it empty.
                ...
                line = indentation + new_key + " :" + value;  // Update the key while preserving any value
            }
        }
 ...
 *
 * This function attempts to extract the value of a specified parameter from a given
 * SDF (Simulation Description Format) element. If the parameter is found and its value
 * is not empty, the value is assigned to the output parameter and the function returns true.
 * Otherwise, an error is logged and the function returns false.
 *
 * @param sdfClone      Pointer to the SDF element to search for the parameter.
 * @param paramName     Name of the parameter to retrieve.
 * @param paramValue    Reference to a string where the parameter value will be stored.
...
bool GetParamFromSDF(sdf::ElementPtr sdfClone, const std::string& paramName, std::string& paramValue) {
    ...
    if (paramValue.empty()) {
        RoboSimLogger::Log(LogLevel::ERR, "gNb_plugin found an empty " + paramName + " parameter. Failed to initialize.");
        return false;
    }
 ...
 * @throws std::invalid_argument If the input string does not represent a valid boolean.
 */
...
/**
 * @brief Extracts the trailing decimal number from the end of the input string and converts it to an uppercase hexadecimal string.
 *
 * This function scans the input string from the end, collects consecutive digits to form a number,
 * converts that number to an integer, and then formats it as a hexadecimal string prefixed with "0x".
 * If no trailing number is found, it returns a message indicating so.
 *
 * @param input The input string potentially ending with a decimal number.
 * @return std::string The hexadecimal representation of the extracted number (e.g., "0x1A3"), or
 *         "No number found at the end of the string" if no trailing number exists.
 */
std::string extractAndConvertToHex(const std::string& input) {
    ...
    for (int i = input.size() - 1; i >= 0; --i) {
        if (isdigit(input[i])) {
            number_str = input[i] + number_str; // Add the digit to the number string
        } else {
    ...
    if (!number_str.empty()) {
        int number = std::stoi(number_str); // Convert the string number to an integer
        
        // Use stringstream to format the number in hexadecimal
        std::stringstream hex_stream;
        hex_stream << "0x" << std::uppercase << std::hex << number; // Convert to hex and make it uppercase
