...
static void triggerDockerCheck(const std::string& name) {
    ...
    // Redirects Docker output to a temporary file, then instantly swaps it. The '&' ensures 0 freezing.
    std::string cmd = "docker exec " + name + " ps -eo pid,stat,args --no-headers 2>/dev/null > " + tmp_path + " && mv " + tmp_path + " " + file_path + " &";
    system(cmd.c_str());
}
...
UePowerPlugin::UePowerPlugin() {
    ...
    connect(status_timer, &QTimer::timeout, this, &UePowerPlugin::checkAndUpdateStatus);
}

UePowerPlugin::~UePowerPlugin() {}

void UePowerPlugin::LoadConfig(const tinyxml2::XMLElement *_pluginElem) {
    if (this->title.empty()) this->title = "UE Power Control";
...
bool UePowerPlugin::isValidFrequency(const std::string &freq) {
    if (freq.empty() || freq.size() < 8 || freq.size() > 12) return false;
    for (char c : freq) { if (!std::isdigit(c)) return false; }
    return true;
}
...
std::vector<int> UePowerPlugin::getProcessPids() const {
    ...
    if (!isValidContainerName(name)) return pids;

    // Reads the file locally. This is instantaneous and immune to Docker lock-ups.
    std::string file_path = "/tmp/ue_" + name + "_pids.txt";
        ...
        while (std::getline(file, line)) {
            ...
            if (iss >> pid_str >> stat) {
                try {
                    ...
                    if (line.find("nr-uesoftmodem") != std::string::npos && stat.front() != 'Z') {
                        pids.push_back(pid);
                    }
                } catch (...) {}
            }
...
std::string UePowerPlugin::buildStartCommand() const {
    ...
    return "docker exec " + name +
           ...
           " -C " + freq + " --rfsimulator.serveraddr " + ip + " > /dev/null 2>&1 &'";
}
...
void UePowerPlugin::checkAndUpdateStatus() {
    ...
    if (is_actually_running) {
        failed_checks = 0;
        ignmsg << "[UePowerPlugin] Polling: PROCESS FOUND (" << live_pids.size() << " PIDs)" << std::endl;
        ...
        failed_checks++;
        ignmsg << "[UePowerPlugin] Polling: PROCESS NOT FOUND (Strike " << failed_checks << "/2)" << std::endl;
...
void UePowerPlugin::toggleProcess() {
    ...
    // --- 1. FIRST CLICK ---
    if (!connected) {
        ignmsg << "[UePowerPlugin] === INITIALIZING CONNECTION ===" << std::endl;
        connected = true;
        
        triggerDockerCheck(name); // Fire the very first check to build the text file
        status_timer->start();
    ...
    // --- 2. SUBSEQUENT CLICKS ---
    skip_checks_count = 3; // Block UI from flickering for 6 seconds while processes boot/die

    if (process_running) {
        ignmsg << "[UePowerPlugin] TURN OFF Clicked. Executing KILL..." << std::endl;
        std::vector<int> current_live_pids = getProcessPids();
        for (int pid : current_live_pids) {
            std::string cmd = "docker exec " + name + " kill -9 " + std::to_string(pid) + " > /dev/null 2>&1 &";
            system(cmd.c_str());
        }
        
        // Immediately delete the text file so the timer doesn't read ghost processes
        system(("rm -f /tmp/ue_" + name + "_pids.txt").c_str());
        
        process_running = false;
        emit processRunningChanged();
    } else {
        ignmsg << "[UePowerPlugin] TURN ON Clicked. Executing START..." << std::endl;
        std::string ip = gnb_ip.toStdString();
        std::string freq = carrier_freq.toStdString();
        if (!isValidIpAddress(ip) || !isValidFrequency(freq)) return;
        
        // Ensure the host doesn't freeze waiting for Docker to parse the command
