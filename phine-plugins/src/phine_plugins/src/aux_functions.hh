...
/**
 * @class RoboSimLogger
 * @brief Provides static logging utilities for RoboSim with support for log levels and colored output.
 *
...
/**
 * @brief Modifies the value associated with a given key in a Docker configuration file.
 *
 * This function searches for the specified key in the Docker configuration file located at `file_path`
 * and updates its value to `new_value`. Optionally, debug information can be printed if `debug` is set to true.
 *
...
// function that takes the gNB name extraxt its number and converts it to hexadecimal to generate the gNB ID
/**
 * @brief Extracts a specific part of the input string and converts it to a hexadecimal representation.
 *
 * This function processes the provided input string, extracts the relevant portion
 * (the extraction logic depends on the implementation), and returns its hexadecimal
 * representation as a std::string.
 *
 * @param input The input string to be processed and converted.
 * @return std::string The hexadecimal representation of the extracted part of the input.
 */
