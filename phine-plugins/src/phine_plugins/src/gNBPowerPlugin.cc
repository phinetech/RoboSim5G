...
gNBPowerPlugin::gNBPowerPlugin() {
    ...
    connect(status_timer, &QTimer::timeout, this, &gNBPowerPlugin::checkAndUpdateStatus);
}
        ...
        while (std::getline(file, line)) {
            ...
            if (iss >> pid_str >> stat) {
                try {
                    ...
                    if (line.find("nr-softmodem") != std::string::npos && stat.front() != 'Z') {
                        pids.push_back(pid);
                    }
                } catch (...) {}
            }
...
std::string gNBPowerPlugin::buildStartCommand() const {
    ...
    return "docker exec " + name +
           ...
           " -E --rfsim"
           " --log_config.global_log_options level,nocolor,time > /dev/null 2>&1 &'";
}
...
void gNBPowerPlugin::checkAndUpdateStatus() {
    ...
    if (is_actually_running) {
        failed_checks = 0;
        ignmsg << "[gNBPowerPlugin] Polling: PROCESS FOUND (" << live_pids.size() << " PIDs)" << std::endl;
        ...
        failed_checks++;
        ignmsg << "[gNBPowerPlugin] Polling: PROCESS NOT FOUND (Strike " << failed_checks << "/2)" << std::endl;
...
void gNBPowerPlugin::toggleProcess() {
    ...
    // --- 1. FIRST CLICK ---
    if (!connected) {
        ignmsg << "[gNBPowerPlugin] === INITIALIZING CONNECTION ===" << std::endl;
        connected = true;
    ...
    if (process_running) {
        ignmsg << "[gNBPowerPlugin] TURN OFF Clicked. Executing KILL..." << std::endl;
        ...
        ignmsg << "[gNBPowerPlugin] TURN ON Clicked. Executing START..." << std::endl;
