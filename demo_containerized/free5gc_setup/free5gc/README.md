...
1. **Check UE has tunnel interface:**
   ...
   ```
   Should see `oaitun_ue1` with IP in `10.0.0.0/24` range
...
3. **Check external data network:**
   ...
   ```
   Should show route to `10.0.0.0/16` via UPF
...
2. Edit `./oai/oai_upf_config.yaml`:
   ...
   upf:
     ue_ip_pools:
       - network: 10.0.0.0/24  # Change this
   ```
...
3. Update the external data network container environment:
   ...
   oai-ext-dn:
     environment:
       - UE_IP_ADDRESS_POOL=10.0.0.0/24 # Change this
   ```
